...
Carrier::Carrier(const AirShipType type,
        ...
        const int         shipId) : ProtossAirShip(type, health, damage, maxShield, shieldRegenerateRate, shipId), _activeInterceptors(MAX_INTERCEPTORS) {}
...
void Carrier::dealDamage(std::vector<std::unique_ptr<AirShip>> & enemyFleet){
    for (int i=0; i < _activeInterceptors; i++) {
        ...
        if (!enemyAirShip->isAlive()) {
            std::cout << "Carrier with ID: " << _shipId << " killed enemy airship with ID: " << enemyAirShip->getAirShipId() << '\n';
            enemyFleet.pop_back();
            if (enemyFleet.empty()) {
                break;
            }
